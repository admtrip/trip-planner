...
// Buckets a trip as current / upcoming / past for the home screen's
// grouping. "Current" has a generous +/-3 day window around the actual
// dates, so a trip still reads as current if you're wrapping up loose ends
// a couple days after the official end date, or started prepping a few
// days before the official start.
const TRIP_STATUS_WINDOW_DAYS = 3
function classifyTrip(trip, today = new Date()) {
  if (!trip.start_date) return 'upcoming' // undated trips have no way to be "past" — keep them visible
  const dayMs = 24 * 60 * 60 * 1000
  const todayMidnight = new Date(today.getFullYear(), today.getMonth(), today.getDate()).getTime()
  const start = new Date(trip.start_date + 'T00:00:00').getTime()
  const end = trip.end_date ? new Date(trip.end_date + 'T00:00:00').getTime() : start
  const windowStart = start - TRIP_STATUS_WINDOW_DAYS * dayMs
  const windowEnd = end + TRIP_STATUS_WINDOW_DAYS * dayMs
...
function App() {
  ...
  // Removes just the current user from a shared trip — for members who
  // aren't the owner. The trip and its data keep existing for everyone else;
  // rejoining later requires a fresh invite. Owners use "Delete trip" from
  // the home screen instead, since leaving wouldn't actually remove it from
  // their own list (it'd still show up under trips they created).
  function leaveTrip(tripId) {
    if (!isOnline) { showInfo({ title: "You're offline", message: 'Leaving a trip requires a connection.' }); return }
    showConfirm({
      title: 'Leave this trip?',
      message: "You'll lose access to its itinerary, expenses, and packing lists unless someone invites you back.",
      confirmLabel: 'Leave trip',
      danger: true,
      onConfirm: async () => {
        const { error } = await supabase.from('trip_members').delete().eq('trip_id', tripId).eq('user_id', user.id)
        if (error) { console.error('Failed to leave trip:', error); showInfo({ title: 'Could not leave trip', message: error.message }); return }
        haptic('success')
  ...
  // General app invite — no trip_id, no invites table row, just a plain
  // link to download/sign up for Trippy itself. Doesn't add the recipient
  // to any trip; they land on their own empty home screen and can be
  // invited to specific trips afterward with generateInviteLink() above.
  async function shareAppInvite() {
    const message = `Come plan trips with me on Trippy!\n\nThe app is in beta, so the first screen you'll see when signing in with your Google Account will say Google hasn't verified this app — that's expected, here's exactly what to do:\n1. Click "Advanced"\n2. Click "Go to biluxvnrawqfsyixhffr.supabase.co (unsafe)"\n3. Click "Continue"\n4. Click "Continue" again\n\niOS app: ${IOS_APP_LINK}\nWeb app: ${WEB_APP_URL}`
    if (navigator.share) {
      try {
        await navigator.share({ title: 'Check out Trippy', text: message })
        return
      } catch (shareErr) {
    ...
    try {
      await navigator.clipboard.writeText(message)
      showInfo({ title: 'Copied!', message: 'Invite message copied to your clipboard.' })
    } catch (clipErr) {
      console.error('Clipboard write failed:', clipErr)
      showInfo({ title: 'Copy this invite manually', message })
    }
  ...
  // Calls a Supabase Edge Function (must be deployed separately — see the
  // delete-account function this ships alongside) that deletes the user's
  // owned trips and all their data, then deletes the auth user itself via
  // the admin API. The anon client can't call supabase.auth.admin.deleteUser
  // directly, so this has to go through a server-side function holding the
  // service role key.
  function deleteAccount() {
    showConfirm({
      title: 'Delete your account?',
      message: "This permanently deletes your account, every trip you created, and removes you from every trip you've joined. This can't be undone.",
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          if (error) {
            ...
            showInfo({ title: 'Could not delete account', message: error.message || 'Please try again, or contact support if this keeps happening.' })
  ...
  if (selectedTrip) {
    ...
    const isTripOwner = selectedTrip.created_by === user.id
        ...
        <div style={{ background: getBannerStyle(selectedTrip.banner_style), padding: 'max(48px, calc(20px + env(safe-area-inset-top))) 24px 32px', position: 'relative' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '14px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', gap: '8px' }}>
              ...
              {activeTab === 'master' && (
                <>
                  <button onClick={() => generateInviteLink(selectedTrip.id)} title="Invite to this trip" style={{
            ...
            <>
              <div style={{ background: 'white', borderRadius: '18px', padding: '16px', marginBottom: '12px', boxShadow: '0 1px 3px rgba(18,18,18,0.05)' }}>
                ...
                {[
                  ...
                  { title: 'Inviting people', body: 'The invite icon on Itinerary (or "Invite someone to this trip" below) only adds someone to THIS trip \u2014 they won\u2019t see any of your other trips. To just point a friend at the app itself, without adding them to any trip, use "Invite a friend to Trippy" on the home screen instead.' },
                  { title: 'Leaving a trip', body: "If you're a member (not the owner) of a trip, you can leave it from Settings \u2014 you'll lose access unless someone invites you back. Trip owners delete the trip entirely from the home screen instead." },
              ...
              <div style={{ background: 'white', borderRadius: '18px', padding: '16px', marginBottom: '12px', boxShadow: '0 1px 3px rgba(18,18,18,0.05)' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                  ...
                  <button onClick={deleteAccount} disabled={deletingAccount} style={{
                    display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', width: '100%', padding: '13px',
                    border: '1.5px solid #A32D2D', borderRadius: '14px', background: 'white', color: '#A32D2D',
                    fontSize: '14px', fontWeight: '700', cursor: deletingAccount ? 'default' : 'pointer', opacity: deletingAccount ? 0.6 : 1, fontFamily: FONT
                  }}>
                    <Trash2 size={16} /> {deletingAccount ? 'Deleting…' : 'Delete my account'}
                  </button>
                </div>
              </div>
            </>
    ...
    <div style={{ minHeight: '100vh', background: BG, fontFamily: FONT }}>
      ...
      <div style={{ maxWidth: '520px', margin: '-24px auto 0', padding: '0 24px 32px' }}>
        ...
        {trips.length === 0 ? (
          ...
          // Most recent to oldest by start date (undated trips sort last
          // within whichever bucket they land in), then split into what's
          // worth seeing right away (current + upcoming) versus a
          // collapsed history of trips that have wrapped up.
          ...
          const activeTrips = sorted.filter(t => classifyTrip(t) !== 'past')
          const pastTrips = sorted.filter(t => classifyTrip(t) === 'past')

          const renderTripCard = (trip) => {
            const status = classifyTrip(trip)
            const statusLabel = status === 'current' ? 'Current' : status === 'upcoming' ? 'Upcoming' : null
            return (
              <div key={trip.id} style={{ position: 'relative', marginBottom: '12px' }}>
                <div onClick={() => setSelectedTrip(trip)} style={{ background: getBannerStyle(trip.banner_style), borderRadius: '24px', padding: '28px 24px', boxShadow: '0 8px 24px rgba(43,52,64,0.3)', textAlign: 'center', cursor: 'pointer' }}>
                  {statusLabel && (
                    <p style={{ fontSize: '10px', fontWeight: '800', letterSpacing: '0.08em', color: 'rgba(214,210,200,0.75)', margin: '0 0 6px', textTransform: 'uppercase' }}>{statusLabel}</p>
                  )}
          ...
          return (
            <>
              {activeTrips.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '32px 24px', color: MUTED, fontSize: '14px', background: 'white', borderRadius: '24px', marginBottom: '12px' }}>
                  No current or upcoming trips — plan a new one above, or check past trips below.
                </div>
              ) : (
              ...
              {pastTrips.length > 0 && (
                <div style={{ marginTop: '16px' }}>
                  <button onClick={() => setShowPastTrips(v => !v)} style={{
                    display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%',
                    background: 'white', border: `1.5px solid ${CARD_BORDER}`, borderRadius: '14px', padding: '13px 16px',
                    cursor: 'pointer', fontFamily: FONT, marginBottom: showPastTrips ? '12px' : 0
                  }}>
                    <span style={{ fontSize: '14px', fontWeight: '700', color: INK }}>Past trips ({pastTrips.length})</span>
                    {showPastTrips ? <ChevronUp size={16} color={MUTED} /> : <ChevronDown size={16} color={MUTED} />}
                  </button>
                  {showPastTrips && pastTrips.map(renderTripCard)}
                </div>
              )}
            </>
          )
